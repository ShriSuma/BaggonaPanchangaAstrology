...
export const FallingCoinsRefillModal: React.FC<FallingCoinsRefillModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl bg-gradient-to-b from-[#FFFDF7] via-[#FFF9E6] to-[#FFF5D6] border-2 border-amber-400 rounded-3xl shadow-2xl p-5 sm:p-7 text-slate-900 my-auto overflow-hidden">
        {/* Close Button */}
        <button
          onClick={() => {
            clearMessages();
          ...
          className="absolute top-4 right-4 text-slate-400 hover:text-slate-800 text-sm font-black p-1.5 rounded-full hover:bg-amber-100 transition-colors z-20"
          aria-label="Close"
        >
          ✕
        </button>

        {/* 1. VISUAL FALLING COINS & GOLDEN KALASHA BANNER */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-900 via-amber-950 to-slate-950 p-4 mb-5 border-2 border-amber-400 shadow-md text-amber-50 flex flex-col sm:flex-row items-center justify-between gap-4">
          {/* Animated Falling Coins Container */}
          ...
          <div className="relative z-10 text-center sm:text-left">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-amber-500/20 border border-amber-400/50 text-amber-300 text-[10px] font-black uppercase tracking-wider mb-1">
              <span>⚡</span>
              <span>॥ ತ್ವರಿತ ನಾಣ್ಯ ರೀಚಾರ್ಜ್ • Instant Coin Refill ॥</span>
            </div>
            <h2 className="text-lg font-black text-amber-200 tracking-tight">
              ನಾಣ್ಯ ಕೋಶ ರೀಚಾರ್ಜ್ (Wallet Refill)
            </h2>
            <div className="flex items-center gap-2 mt-1.5 justify-center sm:justify-start">
              <span className="text-xs text-slate-300 font-semibold">ಪ್ರಸ್ತುತ ಬ್ಯಾಲೆನ್ಸ್:</span>
              <span
          ...
          {/* Right: Golden Vessel / Kalasha Graphic */}
          <div className="relative z-10 flex flex-col items-center">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-amber-600 via-amber-400 to-amber-200 flex items-center justify-center text-3xl shadow-lg border border-amber-300 animate-pulse">
              🏺
            </div>
        ...
        {/* Required Coins Warning if triggered by service */}
        {requiredCoins && requiredCoins > 0 && isZeroOrLow && (
          <div className="mb-4 p-3 bg-red-50 border-2 border-red-400 rounded-2xl flex items-center gap-3 text-xs text-red-950 font-bold shadow-sm">
        ...
        {error && (
          <div className="mb-4 p-3 bg-red-50 border border-red-400 rounded-xl text-red-900 text-xs font-bold flex items-center gap-2">
            <span>⚠️</span>
            <span>{error}</span>
          </div>
        )}
        ...
        {successMessage && (
          <div className="mb-4 p-3 bg-emerald-50 border border-emerald-400 rounded-xl text-emerald-950 text-xs font-bold flex items-center gap-2">
            <span>✅</span>
            <span>{successMessage}</span>
          </div>
        )}

        {/* 2. RECHARGE PACKAGE SELECTOR */}
        <div className="mb-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
            {RECHARGE_PACKAGES.map((pkg: CoinPackage) => {
              ...
              return (
                ...
                >
                  {pkg.tag && (
                    <span className="absolute -top-2 right-2 px-2 py-0.5 text-[8px] font-black uppercase rounded-full bg-amber-500 text-slate-950 border border-amber-300 shadow-sm">
                      {pkg.tag}
                    </span>
                  )}
        ...
        {/* 3. DYNAMIC UPI QR SCANNER & PAYMENT CARD */}
        <div className="bg-[#FEFCF4] border-2 border-amber-300 rounded-2xl p-4 mb-4 shadow-sm">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 justify-between">
            {/* QR Code */}
            <div className="flex flex-col items-center bg-white p-2.5 rounded-2xl border-2 border-amber-300 shadow-md">
              {qrCodeDataUrl ? (
                <img
                  src={qrCodeDataUrl}
                  alt="UPI QR Code"
            ...
            <div className="flex-1 space-y-2.5 w-full">
              <div className="p-2.5 bg-white border border-amber-300 rounded-xl flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-2.5 py-1 bg-amber-100 hover:bg-amber-200 text-amber-950 text-xs font-black rounded-lg border border-amber-300 transition-colors shadow-sm"
                >
              ...
              <a
                href={upiUri}
                className="inline-flex items-center justify-center w-full py-2 px-3 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white font-black rounded-xl text-xs shadow-md transition-all gap-1.5"
              >
        ...
        {/* 4. UTR VERIFICATION SUBMISSION FORM */}
        <form onSubmit={handleUtrSubmit} className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-gradient-to-r from-amber-600 via-amber-500 to-amber-600 hover:from-amber-500 hover:to-amber-400 text-slate-950 font-black text-xs rounded-xl shadow-md disabled:opacity-50 transition-all flex items-center gap-1.5 border border-amber-400"
            >
              {isSubmittingRecharge ? (
