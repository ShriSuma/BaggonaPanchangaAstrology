...
export default function AskAstrologer({ session }: Props): JSX.Element {
  ...
  return (
    <div className="flex flex-col h-[600px] rounded-2xl border border-amber-100 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-xl overflow-hidden animate-fade-in">
      {/* Chat History */}
      ...
      <div className="border-t border-amber-100 dark:border-slate-700 p-4 bg-white dark:bg-slate-800">
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-3 uppercase tracking-wider font-semibold">{t("Select a Deep Insight Query", "Select a Deep Insight Query")}</p>
        <div className="flex flex-wrap gap-2">
          {translatedSuggestions.map((sug) => (
            <button
              ...
              disabled={isTyping}
              className="px-4 py-2 bg-orange-50 hover:bg-amber-100 dark:bg-slate-700 dark:hover:bg-slate-600 text-amber-900 dark:text-amber-100 text-sm font-medium rounded-full transition-colors border border-amber-200 dark:border-slate-600 disabled:opacity-50"
            >
              {sug.label}
            </button>
          ))}
