...
export default function TraditionalSouthPatrika({
  ...
  const hourNum = useMemo(() => {
    ...
    const parsed = parseInt(birthTime.split(":")[0], 10);
    return isNaN(parsed) ? 0 : parsed;
  }, [birthTime]);

  const minNum = useMemo(() => {
    if (!birthTime || !birthTime.includes(":")) return 0;
    const parsed = parseInt(birthTime.split(":")[1], 10);
    return isNaN(parsed) ? 0 : parsed;
  }, [birthTime]);

  const displayH12 = hourNum % 12 || 12;
  ...
  const birthGhatiText = isKn ? (
    `ಅಸ್ಮಿನ್ ಶುಭದಿನೇ ಸೂರ್ಯೋದಯಾದ್ಯಾತ ಘಟಿ ${formatChartHouseNumber(traditionalData.suryodhayadgata.ghati, "kn")}, ವಿಘಟಿ ${formatChartHouseNumber(traditionalData.suryodhayadgata.vighati, "kn")} (${timeOfDayText}) ಘಂಟೆ ${formatChartHouseNumber(displayH12, "kn")} ಮಿ ${formatChartHouseNumber(minNum, "kn")} ತತ್ಕಾಲೇ, ಶ್ರೀ ${RASHI_L5[kundli.lagnaRashi.index] ? pick(RASHI_L5[kundli.lagnaRashi.index], "kn") : t(rashiTKey(kundli.lagnaRashi.sanskrit))} ಲಗ್ನಸ್ಥೇ ಜನ್ಮಕಾಲೇ...`
  ) : (
