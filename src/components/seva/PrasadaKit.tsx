...
export default function PrasadaKit({
  ...
  // When override is not active: default is Shreeram Pandit and 9972339362
  const panditName = useMemo(() => {
    if (overridePriestContact && customPriestName.trim()) {
      return customPriestName.trim();
    }
    return defaultPriest.name[pdfLang as keyof typeof defaultPriest.name] || defaultPriest.name.en || "ಶ್ರೀರಾಮ್ ಪಂಡಿತ್";
  }, [overridePriestContact, customPriestName, defaultPriest, pdfLang]);
  ...
  const handleSelectPriest = (priestId: string) => {
    ...
    if (priestId === "shreeram-pandit") {
      ...
      const locName = p.name[pdfLang as keyof typeof p.name] || p.name.en || p.name.kn;
  ...
  const startPriestVoiceInput = (fieldKey: "name" | "phone", onResult: (val: string) => void) => {
    ...
    try {
      ...
      rec.lang = pdfLang === "kn" ? "kn-IN" : pdfLang === "hi" ? "hi-IN" : pdfLang === "te" ? "te-IN" : pdfLang === "ta" ? "ta-IN" : "en-IN";
  ...
  // Speech Recognition for Custom Pooja Name Mic Button
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <button
                ...
                className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[11px] font-bold transition border ${
              ...
              >
                <span>✏️</span>
                <span>{overridePriestContact ? (lang.startsWith("kn") ? "ಓವರ್‌ರೈಡ್ ಸಕ್ರಿಯ (Active)" : "Override Active") : (lang.startsWith("kn") ? "ವಿವರ ಸಂಪಾದಿಸಿ (Edit)" : "Edit Details")}</span>
              </button>
            </div>
            {!customInputMode ? (
              <div className="flex gap-2 items-center">
            ...
            {!overridePriestContact ? (
              <div className="mt-2 flex items-center justify-between rounded-xl bg-amber-50 p-2.5 border border-amber-300/80 text-xs">
                <div>
                  <div className="font-bold text-amber-950 flex items-center gap-1.5">
                    <span>{defaultPriest.sealSymbol}</span>
                    <span>{defaultPriest.name[pdfLang as keyof typeof defaultPriest.name] || defaultPriest.name.en}</span>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setCustomPriestName(activePriest.name[pdfLang as keyof typeof activePriest.name] || activePriest.name.en);
              ...
              <div className="mt-2 space-y-3 rounded-xl bg-amber-50/90 p-3.5 border border-amber-300/90 shadow-sm">
                ...
                <p className="text-[10px] text-amber-800/90 font-medium">
                  {lang.startsWith("kn")
                    ? "⚡ ಓವರ್‌ರೈಡ್ ಮಾಡಲಾದ ಹೆಸರು ಮತ್ತು ದೂರವಾಣಿ ಸಂಖ್ಯೆಯು ಆಶೀರ್ವಾದ ಪತ್ರ, QR ಕೋಡ್ ಮತ್ತು ದೈನಂದಿನ ದರ್ಶನ ಪುಟದಲ್ಲಿ (9972339362 ಬದಲಿಗೆ) ಪ್ರತಿಫಲಿಸುತ್ತದೆ."
                    : "⚡ Overridden name & phone will appear on Seva Ashirvada Letter, QR code, and Daily Darshana call button (instead of 9972339362)."}
                </p>
              </div>
            )}
          </div>
