...
type SubTab = "personal" | "overview" | "planets" | "houses" | "yogas" | "gochara" | "muhurtha";
...
export default function BaggonaPredictionsPage(): JSX.Element {
  ...
  const [selectedMuhurthaCategory, setSelectedMuhurthaCategory] = useState<"all" | "marriage" | "housewarming" | "upanayana" | "general">("all");
  ...
  useEffect(() => {
    if (tab !== "muhurtha" || !record) return;
    let active = true;
    setLoadingMuhurthas(true);
    const timer = setTimeout(() => {
      fetchShuddhaMuhurthas(selectedMuhurthaYear, record.latitude, record.longitude, ayanamsaModel)
        .then((res) => {
          if (active) {
            setMuhurthas(res);
  ...
  }, [tab, selectedMuhurthaYear, record, ayanamsaModel]);
  ...
  const getTabLabel = (id: SubTab): string => {
    switch (id) {
      ...
      case "muhurtha":
        if (lang === "kn") return "ಶುಭ ಮುಹೂರ್ತ";
        if (lang === "hi") return "शुभ मुहूर्त";
        if (lang === "ta") return "சுப முகூர்த்தம்";
        if (lang === "te") return "శుభ ముహూర్తం";
        return "Muhurtham";
    }
  };
  ...
  const subTabs: { id: SubTab; label: string; icon: string }[] = [
    ...
    { id: "muhurtha", label: getTabLabel("muhurtha"), icon: "🔔" }
  ];
          ...
          {tab === "muhurtha" && (
            <div className="space-y-6 animate-fade-in">
                  ...
                  <div className="flex-1 sm:max-w-xl">
                    ...
                    <div className="flex flex-wrap gap-1.5 justify-start sm:justify-end">
                      {(["all", "marriage", "housewarming", "upanayana", "general"] as const).map((cat) => {
                        const label =
                          cat === "all" ? (isKn ? "ಎಲ್ಲಾ" : "All") :
                          cat === "marriage" ? (isKn ? "ವಿವಾಹ 💍" : "Marriage 💍") :
                          cat === "housewarming" ? (isKn ? "ಗೃಹಪ್ರವೇಶ 🏡" : "Housewarming 🏡") :
                          cat === "upanayana" ? (isKn ? "ಉಪನಯನ 🪘" : "Upanayana 🪘") :
              ...
              {loadingMuhurthas ? (
                ...
                const filtered = muhurthas.filter((m) => {
                        ...
                        >
                          ...
                          <div className="p-4 space-y-2.5">
                            <div className="flex justify-between items-center text-xs">
                              ...
                              <span className="font-semibold text-slate-900">{isKn ? m.nakshatraNameKn : m.nakshatraName}</span>
                            </div>
                            <div className="flex justify-between items-center text-xs">
                              <span className="text-slate-500">{isKn ? "ತಿಥಿ" : "Tithi"}</span>
                              <span className="font-semibold text-slate-900">{isKn ? m.tithiNameKn : m.tithiName}</span>
                            </div>
                            ...
                            {isExpanded && (
                              <div className="mt-3 pt-3 border-t border-black/5 space-y-2 text-[11px] leading-relaxed text-slate-700 animate-slide-down">
                                ...
                                <div className="flex justify-between">
                                  ...
                                  <span className="font-medium text-slate-900">{isKn ? m.weekdayNameKn : m.weekdayName}</span>
                                </div>
