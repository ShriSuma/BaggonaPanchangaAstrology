...
export function generateSevaICalendarString(options: CalendarGeneratorOptions): string {
  ...
  alignedDays.forEach((day, idx) => {
    ...
    const viewBhavishyaButtonLabel = isKn
      ? "ದಿನದ ಸಂಪೂರ್ಣ ಭವಿಷ್ಯವನ್ನು ತಿಳಿದುಕೊಳ್ಳಿ"
      : isTe
      ? "దిన సంపూర్ణ భవిష్యత్తును తెలుసుకోండి"
      : isTa
      ? "நாளின் முழுமையான பலனைத் தெரிந்துகொள்ளுங்கள்"
      : isHi
      ? "दिन का संपूर्ण भविष्य जानें"
      : "Know Today's Complete Horoscope";

    const bellDarshanaLabel = isKn
      ? "🔔 ಗಂಟಾನಾದ & ದರ್ಶನ"
      : isHi
      ? "🔔 घंटानाद एवं दर्शन"
      : isTe
      ...
      : isTa
      ? "🔔 மணியோசை & தரிசனம்"
      : "🔔 Temple Bell & Darshana";

    const dailyDarshanaAlarmLabel = isKn
      ? "🔔 ದೈನಂದಿನ ದರ್ಶನ"
      : isHi
      ? "🔔 दैनिक दर्शन"
      : isTe
    ...
    const morningReminderAlarmLabel = isKn
      ? "🔔 ಮುಂಜಾನೆ ಪಂಚಾಂಗ ಜ್ಞಾಪನೆ"
      : isHi
      ? "🔔 प्रभात पंचांग स्मरण"
      : isTe
      ? "🔔 ఉదయకాల పంచాంగ జ్ఞాపిక"
      : isTa
      ? "🔔 காலை பஞ்சாங்க நினைவೂಟಲ್"
      : "🔔 Morning Panchanga Reminder";

    const descriptionParts: string[] = [
      openSanctumPrompt,
      `<a href="${sanctumUrl}"><b>🔮 [ ${viewBhavishyaButtonLabel} ] ➔</b></a>`,
      "",
      sanctumUrl,
      "",
      "════════════════════════════════════════",
    ...
    const eventLines: string[] = [
      ...
      `DESCRIPTION:${escapeIcsText(`${bellDarshanaLabel}: ${summaryStr}`)}`,
      "TRIGGER:-PT0M",
      "REPEAT:2",
      "DURATION:PT2M",
      "END:VALARM",
      "BEGIN:VALARM",
      "ACTION:DISPLAY",
      `DESCRIPTION:${escapeIcsText(`${dailyDarshanaAlarmLabel}: ${summaryStr}`)}`,
      "TRIGGER:-PT0M",
      "END:VALARM",
      "BEGIN:VALARM",
      "ACTION:DISPLAY",
      `DESCRIPTION:${escapeIcsText(`${morningReminderAlarmLabel}: ${summaryStr}`)}`,
      "TRIGGER:-PT15M",
      "END:VALARM",
      "END:VEVENT"
    ];
...
export function generateGoogleCalendarUrl(options: {
  ...
  const directTapLabel = isKn
    ...
    ? "👉 सीधे लाइव दर्शन के लिए नीचे दिए लिंक पर क्लिक करें (CLICK TO OPEN):"
    : isTe
    ? "👉 ప్రత్యక్ష దర్శనం కొరకు క్రింది లింక్‌పై క్లిక్ చేయండి (CLICK TO OPEN):"
    : isTa
    ? "👉 நேரடி தரிசனத்திற்கு கீழே உள்ள இணைப்பை கிளிக் செய்யவும் (CLICK TO OPEN):"
    : "👉 Click the link below to enter Live Darshana (CLICK TO OPEN):";
  ...
  const buttonCallToAction = `${viewBhavishyaButtonLabel} - ${openActionText}`;

  const details = [
    directTapLabel,
    `<a href="${sanctumUrl}"><b>🔮 [ ${buttonCallToAction} ] ➔</b></a>`,
    "",
    sanctumUrl,
    "",
    "════════════════════════════════════════",
