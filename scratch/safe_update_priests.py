...
target_split = "export function getHardcodedPoojaVidhiDetails"
parts = content.split(target_split)

if "narayanabali_tripindi:" not in parts[0]:
    # Replace the last `};` in parts[0]
    last_brace_idx = parts[0].rfind("};")
    if last_brace_idx != -1:
        parts[0] = parts[0][:last_brace_idx] + combined_vidhis + "\n\n"
        content = parts[0] + target_split + parts[1]
        with open(file_priests, "w", encoding="utf-8") as f:
            f.write(content)
        print("Successfully updated sevaPriestDirectory.ts!")
    else:
