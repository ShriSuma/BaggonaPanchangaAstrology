...
# 1. Helper dictionaries to be added before component or inside file top
helper_code = '''
...
const NAKSHATRA_DEITIES: Record<number, { deity: Record<string, string>; symbol: Record<string, string>; mantra: string }> = {
  ...
  10: { deity: { kn: "ಭಗ ದೇವ (ಸೂರ್ಯ ಸಂಭೂತ)", en: "Bhaga (Solar God)", hi: "भग देव", te: "భగ దేవుడు", ta: "பக தேவன்" }, symbol: { kn: "ಮಂಚದ ಮುಂಭಾಗ", en: "Front Legs of Couch", hi: "शय्या", te: "మంచం", ta: "கட்டில்" }, mantra: "ॐ ಭಗಾಯ ನಮಃ" },
  11: { deity: { kn: "ಅರ್ಯಮನ್ ದೇವ", en: "Aryaman (Patron God)", hi: "अर्यमा देव", te: "अर्यमुकार", ta: "அர்யமா தேவன்" }, symbol: { kn: "ಮಂಚದ ಹಿಂಭಾಗ", en: "Back Legs of Couch", hi: "शय्या", te: "మంచం", ta: "கட்டில்" }, mantra: "ॐ ಅರ್ಯಮ್ಣೇ ನಮಃ" },
  ...
  16: { deity: { kn: "ಮಿತ್ರ ದೇವ", en: "Mitra (God of Friendship)", hi: "मित्र देव", te: "मिश्र देव", ta: "மித்ர தேவன்" }, symbol: { kn: "ಕಮಲದ ಹೂವು", en: "Lotus Flower", hi: "कमल पुष्प", te: "పద్మం", ta: "தாமரை" }, mantra: "ॐ ಮಿತ್ರಾಯ ನಮಃ" },
  ...
  22: { deity: { kn: "ಅಷ್ಟ ವಸುಗಳು", en: "Ashta Vasus (Eight Gods)", hi: "अष्ट वसु", te: "అష్ట వసువులు", ta: "அஷ்ட வசுக்கள்" }, symbol: { kn: "ಮೃದಂಗ / ಡಮರು", en: "Drum / Flute", hi: "डमरू / मृदंग", te: "మృదంగం", ta: "மிருதங்கம்" }, mantra: "ॐ ವಸುಭ್ಯೋ ನಮಃ" },
  ...
  25: { deity: { kn: "ಅಹಿರ್ಬುಧ್ನ್ಯ (ಅನಂತ ನಾಗ)", en: "Ahirbudhnya (Cosmic Snake)", hi: "अहिर्बुध्न्य", te: "అహిర్బుధ్న్యుడు", ta: "அஹிர்புத்னியர்" }, symbol: { kn: "ನಾಗ ಕುಂಡಲ / ಜಲ ಶಯನ", en: "Snake in Water", hi: "शेषनाग", te: "శేషనాగు", ta: "சேஷநாகம்" }, mantra: "ॐ ಅಹಿರ್ಬುಧ್ನ್ಯಾಯ ನಮಃ" },
...
# Insert helper code right before `export const RoyalBooklet8PageTemplate`
...
# Replace the Page 1 predictions block (lines 389 - 432 in template) with new 3 sections
old_p1_block = re.search(r'\{\/\* 6 Life Predictions Grid.*?\n\s*<\/div>\n\s*<\/div>', content, re.DOTALL)

new_p1_block = '''{/* NEW SECTION 1: Panchanga Birth Pillars (2x2 Grid) */}
          <div style={{
            background: "#FFFDF7",
...
if old_p1_block:
    content = content.replace(old_p1_block.group(0), new_p1_block)

# Clean up Devotee record box on Page 1 to use target language strings from PAGE1_LABELS
content = content.replace(
    '<div><span style={{ color: "#D97706" }}>🔸</span> <strong style={{ color: "#B45309" }}>ಜನ್ಮ ರಾಶಿ:</strong> {rashiName}</div>',
    '<div><span style={{ color: "#D97706" }}>🔸</span> <strong style={{ color: "#B45309" }}>{(PAGE1_LABELS[code] || PAGE1_LABELS.en).labelRashi}:</strong> {rashiName}</div>'
)
