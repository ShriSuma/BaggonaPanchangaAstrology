...
# Replace month title format string in page6Data
old_title_code = 'const mTitle = isKn ? `${toKnDigits(i + 1)} ನೇ ತಿಂಗಳು (${mName} ${toKnDigits(yr)})` : `Month ${i + 1} (${mName} ${yr})`;'

new_title_code = '''const knOrdinals = [
      "ಒಂದನೇ ತಿಂಗಳು",
      "ಎರಡನೇ ತಿಂಗಳು",
      "ಮೂರನೇ ತಿಂಗಳು",
      "ನಾಲ್ಕನೇ ತಿಂಗಳು",
...
# Replace themesKn array to expand f1 to 2 full lines
old_themesKn_marker = 'const themesKn = ['
old_themesEn_marker = 'const themesEn = ['

s_idx = content.find(old_themesKn_marker)
e_idx = content.find(old_themesEn_marker)

print(f"s_idx: {s_idx}, e_idx: {e_idx}")

new_themesKn = '''const themesKn = [
      {
        badge: "💼 ವೃತ್ತಿ ಪ್ರಗತಿ",
...
if s_idx != -1 and e_idx != -1:
    content = content[:s_idx] + new_themesKn + content[e_idx:]
    with open(filepath, "w", encoding="utf-8") as f:
        f.write(content)
    print("Replaced themesKn and month titles successfully.")
